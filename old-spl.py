...
class NDF_Element:
	def __init__(self,name, code, id, kind,properties):
		self.name = name
		self.code = code
		self.id = id
		if kind == 'C8':
			...
			self.kind = 'PHYSIOLOGICAL EFFECT'
		elif kind == 'C10':
			self.kind = 'INGREDIENT'
		elif kind == 'C12':
			self.kind = 'MECHANISM OF ACTION'
		elif kind == 'C14':
		...
		for mprop in property_definition:

			for propItem in self.properties:
				if mprop == propItem:
					table_prop = property_definition[mprop]
					self.med_properties[table_prop] = self.properties[propItem]


		for role in role_definition:
			for propItem in self.properties:
				if role == propItem:
					self.med_properties[role_definition[role]]=self.properties[propItem]
...
def read_ndfrt():
	for child in root:
	if child.tag == 'propertyDef' or child.tag == 'roleDef' or child.tag == 'kindDef':
		...
		if child.tag == 'propertyDef':
			property_definition[code_key] = name_key
		elif child.tag == 'roleDef':
			role_definition[code_key] = name_key
		elif child.tag == 'kindDef':
			kind_definition[code_key] = name_key
	...
	if child.tag == 'propertyDef' or child.tag == 'roleDef' or child.tag == 'kindDef':
		...
		propObj = NDF_Element(name,code,id,kind,item_properties)
		term_objects.append(propObj)
